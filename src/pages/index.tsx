...
export default function Component() {
  ...
  const router = useRouter()  // Initialize useRouter
   ...
   useEffect(() => {
    ...
    const redirect = ()=>{
      ...
      if(userUid){
        setTimeout(()=>  router.push('/scheduler') , 500)
      }
  ...
  <Card className="w-full max-w-md mx-auto backdrop-blur-sm bg-white/20 border border-purple-700 shadow-2xl">
    <CardHeader className="space-y-1">
      <CardTitle className="text-2xl font-bold text-center text-yellow-300">Login</CardTitle>
  ...
  <p className="text-sm text-gray-300">
    <Link href="/forgot-password" className="text-yellow-300 hover:underline font-semibold">
      Forgot Password?
    </Link>
  </p>
